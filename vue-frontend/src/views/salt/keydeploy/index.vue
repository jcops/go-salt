<template>
  <div class="edit-documents app-container">
    <el-form>
      <div class="el-col-24">
        <el-form-item label="节点" label-width="65px" prop="title">
          <el-input
            v-model="textarea.ip"
            type="textarea"
            :rows="3"
            placeholder="输入IP"
          />
        </el-form-item>
        <el-form-item label="组件" label-width="65px" prop="title">
          <el-radio-group v-model="textarea.radio">
            <el-radio :label="1">Redis</el-radio>
            <el-radio :label="2">Nginx</el-radio>
            <el-radio :label="3">系统初始化</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="操作" label-width="65px">
          <el-button
            type="primary"
            size="small"
            @click="handleDeploySubmit"
          >执行安装
          </el-button>
        </el-form-item>
      </div>
      <div class="el-col-24">
        <el-form-item v-loading="CreateLoading" label="结果" label-width="65px" prop="title">
          <el-input
            type="textarea"
            :rows="3"
            v-html="textarea.res"
          />
        </el-form-item>
      </div>
    </el-form>
  </div>
</template>
<script>
import { saltdeploy } from '@/api/salt'
export default {
  name: 'Index',
  data() {
    return {
      CreateLoading: false,
      textarea: {
        ip: '',
        radio: 3,
        res: []
      }
    }
  },
  methods: {
    handleDeploySubmit() {
      console.log(this.match)
      if (this.textarea.radio === 1) {
        this.textarea.radio = 'redis'
      } else if (this.textarea.radio === 2) {
        this.textarea.radio = 'nginx'
      } else {
        this.textarea.radio = 'system_init'
      }
      console.log(this.textarea.radio)
      this.$confirm(`安装组件, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.CreateLoading = true
        saltdeploy({ match: this.textarea.ip, arg: this.textarea.radio }).then(res => {
          if (res.msg === 'ok') {
            this.CreateLoading = false
            console.log(res)
            this.textarea.res = res.data.data
            // this.textarea.res = JSON.stringify(res.data.data, null, 4)
            this.$notify({
              title: '成功',
              message: '成功',
              type: 'success'
            })
          }
        })
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消'
        })
      })
    }
  }
}

</script>
